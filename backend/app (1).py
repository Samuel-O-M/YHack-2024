...
LATEX_UPLOAD_FOLDER = 'uploads/latex'

os.makedirs(IMAGE_UPLOAD_FOLDER, exist_ok=True)
os.makedirs(LATEX_UPLOAD_FOLDER, exist_ok=True)
...
def upload_files():
    """Endpoint to receive images from a zip file and a LaTeX text file."""
    ...
    # Create a unique temporary directory for storing the zip file images
    unique_id = uuid.uuid4().hex
    image_temp_dir = os.path.join(IMAGE_UPLOAD_FOLDER, unique_id)
    os.makedirs(image_temp_dir, exist_ok=True)

    # Save images from the zip file to the image directory
    for image in images:
        if image.filename == '':
            continue  # Skip empty filenames
        if image and allowed_file(image.filename, ALLOWED_IMAGE_EXTENSIONS):
            image_filename = secure_filename(image.filename)
            image_path = os.path.join(image_temp_dir, image_filename)
            image.save(image_path)
        else:
            return f'Invalid image file type: {image.filename}', 400

    # Save the LaTeX text file to the LaTeX directory
    ...
    if text_file and allowed_file(text_file.filename, ALLOWED_TEXT_EXTENSIONS):
        ...
        latex_file_path = os.path.join(LATEX_UPLOAD_FOLDER, text_filename)
        text_file.save(latex_file_path)
    else:
        return 'Invalid text file type', 400

    # Change current working directory to the latex folder
    original_cwd = os.getcwd()
    os.chdir(LATEX_UPLOAD_FOLDER)
